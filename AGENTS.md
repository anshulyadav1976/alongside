...
- `lib/memory/**`
- `lib/safety/**`
- `data/**` runtime schema/seed paths and SQLite migrations if present
- `tests/backend/**`
...
```text
feat(voice): add local audio turn and TTS route
...
- Raw audio is short-lived and deleted after canonical transcription where the provider path permits it.
- Secrets and provider credentials are server-only.
- The local demo has one fixed `DEMO_USER_ID`; authentication is intentionally out of scope.
