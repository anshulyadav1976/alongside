...
|---|---|---|---|
| `S0-foundation` | Scaffold server paths, SQLite database, env validation, contracts, seed-user strategy | Scaffold app shell, design tokens, navigation, mock service, shared types consumption | App boots; mocked dashboard renders; health endpoint passes |
| `S1-voice` | Local audio-turn route, GPT transcription/response, ElevenLabs TTS streaming, session metadata | Call screen, microphone consent, recording/transcribing/thinking/speaking states, transcript panel | A test turn starts, ends, plays audio, and creates a session |
...
| `S6-hardening-demo` | Safety gate, idempotency, SQLite integrity checks, seed scripts, backend evals | Product QA, accessibility, empty/error states, demo story and pitch | Full three-call demo passes twice without manual repair |
