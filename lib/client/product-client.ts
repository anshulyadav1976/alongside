...
const apiClient: ProductClient = {
  ...
  async sendTurn(sessionId, audio) { if (!audio) throw new Error("No microphone recording was captured."); const body = new FormData(); body.set("audio", audio, audio.type.includes("wav") ? "turn.wav" : "turn.mp3"); const result = await request<ApiRow>(`/calls/${sessionId}/turn`, { method: "POST", body }); const audioError = result.audioError as ApiRow | undefined; return { sessionId: text(result.sessionId, sessionId), turnId: text(result.turnId), userTranscript: text(result.userTranscript), assistantText: text(result.assistantText), audioUrl: result.audioUrl ? text(result.audioUrl) : undefined, audioError: audioError ? { code: audioError.code ? text(audioError.code) : undefined, message: text(audioError.message) } : undefined, transcriptSource: text(result.transcriptSource, "demo") as TurnResponse["transcriptSource"] }; },
