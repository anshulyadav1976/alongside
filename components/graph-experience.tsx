...
const positions = [{ x: 60, y: 210 }, { x: 340, y: 70 }, { x: 340, y: 350 }, { x: 650, y: 205 }, { x: 920, y: 75 }, { x: 920, y: 360 }];
...
export function GraphExperience() {
  ...
  const nodes = useMemo<Node<FlowData>[]>(() => filteredNodes.map((item, index) => ({ id: item.id, type: "memory", position: positions[index % positions.length], data: { item, onSelect: setSelected } })), [filteredNodes]);
  const edges = useMemo<Edge[]>(() => (graph?.edges.filter((edge) => filteredNodes.some((node) => node.id === edge.source) && filteredNodes.some((node) => node.id === edge.target)).map((edge) => ({ id: edge.id, source: edge.source, target: edge.target, label: edge.predicate.replace("_", " "), type: "smoothstep", animated: false })) ?? []), [graph, filteredNodes]);
  ...
  return <div className="graph-page"><header className="graph-head"><div><p className="eyebrow">Your memory map</p><h1>What you chose to keep, in context.</h1><p>The current view shows what applies now. History keeps changes visible without treating the past as the present.</p></div><div className="legend"><span><i className="confirmed" /> Confirmed</span><span><i className="inferred" /> Inference</span><span><i className="historical" /> Historical</span></div></header>
    <section className="graph-toolbar"><div className="segmented" role="group" aria-label="Graph time view"><button className={view === "current" ? "active" : ""} onClick={() => setView("current")}><Eye size={15} /> Current</button><button className={view === "history" ? "active" : ""} onClick={() => setView("history")}><History size={15} /> History</button></div><label className="filter-select"><Filter size={15} /><span className="sr-only">Filter node type</span><select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)}><option value="all">All types</option><option value="event">Events</option><option value="strategy">Strategies</option><option value="boundary">Boundaries</option><option value="preference">Preferences</option></select></label></section>
