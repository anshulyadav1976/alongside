...
export function SessionExperience({ sessionId }: { sessionId: string }) {
  ...
  useEffect(() => { productClient.getSession(sessionId).then(async (nextSession) => { const [nextJournal, nextTranscript] = await Promise.all([productClient.getJournal(nextSession.journalId ?? ""), productClient.getTranscript(sessionId)]); setSession(nextSession); setJournal(nextJournal); setTranscript(nextTranscript); setMemories(nextJournal.candidateMemories); }); }, [sessionId]);
