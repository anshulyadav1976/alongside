...
- Audio turn returns a canonical transcript and assistant response.
- Duplicate turn requests are idempotent.
- Provider failure falls back to a deterministic demo response.
- TTS failures do not lose the transcript or response text.
...
1. Start voice call.
2. End call.
3. Each audio turn is accepted once.
